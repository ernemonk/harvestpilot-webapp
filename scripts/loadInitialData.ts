...
if (!userId) {
  ...
  console.log('Usage: npm run load-data YOUR_USER_ID');
  process.exit(1);
}
  ...
  for (const record of records) {
    try {
      ...
      const cropData = {
        ...
        // Dummy IDs for fieldId and sectionId - you can update these later
      ...
      errorCount++;
      console.error(`❌ Failed to add ${record.name}:`, error);
    }
  }

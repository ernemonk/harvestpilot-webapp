...
interface MigrationStats {
  ...
  cropsUpdated: number;
  harvestsUpdated: number;
  customersUpdated: number;
  fieldsUpdated: number;
  cropResearchUpdated: number;
  errors: string[];
}

export async function migrateToMultiTenant(): Promise<MigrationStats> {
...
async function migrateUserData(
  ...
  const cropsSnap = await getDocs(
    query(collection(db, 'crops'), where('userId', '==', userId))
  );
  ...
  // Migrate Harvests
  const harvestsSnap = await getDocs(
    query(collection(db, 'harvests'), where('userId', '==', userId))
  );
  ...
  const researchSnap = await getDocs(
    query(collection(db, 'crop_research'), where('userId', '==', userId))
  );

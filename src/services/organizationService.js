...
export const organizationService = {
    ...
    async createOrganization(name, ownerId, ownerEmail, ownerName) {
        const orgData = {
            name,
            type: 'farm',
            ownerId,
        ...
        await this.addMember(orgRef.id, ownerId, ownerEmail, ownerName, 'owner', ownerId);
    ...
    async getUserOrganizations(userId) {
        ...
        const memberQuery = query(collection(db, MEMBER_COLLECTION), where('userId', '==', userId), where('status', '==', 'active'));
    ...
    async addMember(organizationId, userId, userEmail, userName, role, invitedBy, status = 'active') {
    ...
    async getOrganizationMembers(organizationId) {
        const q = query(collection(db, MEMBER_COLLECTION), where('organizationId', '==', organizationId));
        const snapshot = await getDocs(q);
        return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    },
    // Get a specific member
    async getMember(userId, organizationId) {
        const memberId = `${userId}_${organizationId}`;
    ...
    async inviteMember(organizationId, email, role, invitedBy) {
    ...
    async acceptInvitation(inviteToken, userId, userName) {
        const q = query(collection(db, MEMBER_COLLECTION), where('inviteToken', '==', inviteToken), where('status', '==', 'invited'));
        const snapshot = await getDocs(q);
        if (snapshot.empty) {
        ...
        await this.addMember(inviteData.organizationId, userId, inviteData.userEmail, userName, inviteData.role, inviteData.invitedBy, 'active');
